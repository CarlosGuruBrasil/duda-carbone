...
export default function Hero() {
  ...
  return (
    <section
      ...
      style={{ background: BG }}
    >
      {/* Foto da Duda — ancorada à direita em TODOS os breakpoints */}
      <div
        className="absolute inset-y-0 right-0 z-0"
        style={{ width: "58%", maxWidth: "700px" }}
        aria-hidden
      >
        <Image
          src="/duda_hero.png"
        ...
        <div
          ...
          style={{ background: `linear-gradient(to bottom, ${BG}BB, ${BG}00)` }}
        />
      </div>

      {/* Lime glow */}
      <div
        className="absolute bottom-0 right-0 w-[350px] h-[350px] pointer-events-none z-0"
        style={{ background: "radial-gradient(ellipse, oklch(0.84 0.22 130 / 0.07) 0%, transparent 65%)" }}
        aria-hidden
      />

      {/* Conteúdo — coluna esquerda */}
      <div className="relative z-10 flex-1 flex flex-col px-5 md:px-12 w-full max-w-7xl mx-auto">
        ...
        {/* Nome + conquistas — max 52% para não cobrir a Duda */}
        <div
          className="flex-1 flex flex-col justify-center py-4 md:py-8"
          style={{ maxWidth: "min(52%, 560px)" }}
        >
          <motion.div
          ...
          >
            ...
            >
              <a
                ...
                style={{ padding: "0.6rem 1rem", fontSize: "clamp(0.58rem, 1.5vw, 0.75rem)", letterSpacing: "0.1em" }}
              >
                Patrocinar
              </a>
              <a
                ...
                style={{ padding: "0.6rem 0.9rem", fontSize: "clamp(0.58rem, 1.5vw, 0.75rem)", letterSpacing: "0.1em" }}
            ...
            {stats.map((s, i) => (
              ...
              >
                <div
                  ...
                  style={{ fontSize: "clamp(1.2rem, 3.8vw, 3rem)", color: "oklch(0.84 0.22 130)" }}
                >
                  {s.v}
                </div>
                <div
                  className="font-display font-semibold tracking-widest uppercase"
                  style={{ fontSize: "clamp(0.45rem, 1.1vw, 0.6rem)", color: "oklch(0.50 0.010 260)" }}
                >
                  {s.l}
                </div>
              </motion.div>
